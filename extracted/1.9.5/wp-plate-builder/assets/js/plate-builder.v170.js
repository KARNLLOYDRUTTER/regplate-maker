...
(function () {
  ...
  function initPlateBuilder(container) {
    ...
    const designImageEl = container.querySelector('.design-image');
...
const textY = strokeOffsetTotal + marginTopPx + charHeightPx / 2;
    ...
    function updatePrice() {
      ...
      let price = 0;
      if (count === 'both') {
        price += (parseFloat(fb.front) || 0) + (parseFloat(fb.rear) || 0);
      } else if (count === 'front') {
        price += (parseFloat(fb.front) || 0);
      } else {
        price += (parseFloat(fb.rear) || 0);
      }

      const sz = (sizeKey && sizePricesFR[sizeKey]) ? sizePricesFR[sizeKey] : { front: 0, rear: 0 };
      if (count === 'both') {
        price += (parseFloat(sz.front) || 0) + (parseFloat(sz.rear) || 0);
      } else if (count === 'front') {
        price += (parseFloat(sz.front) || 0);
      } else {
        price += (parseFloat(sz.rear) || 0);
      }
    ...
    // duplicate updatePrice implementation removed
    ...
    function updateDesignImage() {
      if (!designImageEl) return;
      const style = textStyleSelect ? textStyleSelect.value : null;
      const url = style && styleImages[style] ? styleImages[style] : '';
      if (url) { designImageEl.src = url; designImageEl.style.display = 'block'; }
      else { designImageEl.removeAttribute('src'); designImageEl.style.display = 'none'; }
    }
