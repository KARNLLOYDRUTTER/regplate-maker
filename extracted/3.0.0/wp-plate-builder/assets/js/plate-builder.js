/* WP Plate Builder JS v1.7.0 */
/* WP Plate Builder JS v1.6.0 */
/* WP Plate Builder JS v1.5.9 */
...
(function () {
  ...
  function initPlateBuilder(container) {
    ...
    const platePreview = container.querySelector('.plate-preview');
    const plateOptions = container.querySelector('.plate-options');
    ...
    function renderSVG(svgEl, isRear, dims) {
      ...
      const isHex = /hex/i.test(sizeKey);
      const arrowLen = isHex ? Math.max(width * 0.18, height * 0.9) : 0; // 18% of width or >= height*0.9
      const corner = Math.max(height * 0.09, 6); // corner radius for rects
...
// Compute badge strip dimensions. The badge occupies 15% of the plate's
      ...
      // Inline style for the registration text nodes
      const filterAttr = '';
...
// Build tspans for each character, positioning each at the
      ...
      function buildTspansFor(text) {
        ...
        for (let i = 0; i < text.length; i++) {
          ...
          // Optical spacing: narrower advance for '1' and 'I' to reduce perceived gaps
          const cw = (ch === '1' || ch === 'I') ? charWidthPx * 0.82 : charWidthPx;
          const cx = xCurLocal + cw / 2;
...
const textY = strokeOffsetTotal + marginTopPx + charHeightPx / 2;
    ...
    (function(){ try {
      ...
      if (wrap) {
        ...
        var tweakMmMap = { 'large_rear': 1.5, 'short6': 1.5, 'short5': 1.5, 'short4': 1.5, 'hex7': 0.5, 'hex6': 0.5, 'hex5': 0.5, 'hex4': 0.5 };
    ...
    const stylePricesFB = config.stylePricesFB || {};
    const sizePricesFR  = config.sizePricesFR || {};
    const extrasConfig  = config.extras || {};
    const styleImages   = config.styleImages || {};
    ...
    function updateRegistration() {
      ...
      // Auto-format common modern UK style (AB12 CDE) when typed without a space
      let regForPreview = reg;
      if (/^[A-Z]{2}[0-9]{2}[A-Z]{3}$/.test(reg.replace(/\s+/g,''))) {
        const compact = reg.replace(/\s+/g,'');
        regForPreview = compact.slice(0,4) + ' ' + compact.slice(4);
      }
      const text = regForPreview || 'YOUR REG';
    ...
    function ensureExtrasUI() {
      if (!plateOptions) return;
      if (plateOptions.querySelector('.extras-section')) return;
      const section = document.createElement('div');
      section.className = 'option-section extras-section';
      section.innerHTML = '<div class="option-header">Extras</div><div class="option-content extras-content"></div>';
      const content = section.querySelector('.extras-content');
      const keys = Object.keys(extrasConfig || {});
      if (!keys.length) {
        content.innerHTML = '<em>No extras configured.</em>';
      } else {
        keys.forEach(k => {
          const ex = extrasConfig[k] || {};
          const label = document.createElement('label');
          label.innerHTML = '<input type="checkbox" class="extra-option" value="'+k+'"> ' + (ex.label || k);
          content.appendChild(label);
          content.appendChild(document.createElement('br'));
        });
      }
      plateOptions.appendChild(section);
      plateOptions.querySelectorAll('.extra-option').forEach(chk => chk.addEventListener('change', updatePrice));
    }

    function ensureDesignImage() {
      if (!platePreview) return;
      if (platePreview.querySelector('.design-image')) return;
      const div = document.createElement('div');
      div.className = 'design-image-section';
      div.style.cssText = 'margin-top:10px;text-align:center;';
      const img = document.createElement('img');
      img.className = 'design-image';
      img.style.cssText = 'max-width:100%;height:auto;display:none;';
      div.appendChild(img);
      platePreview.appendChild(div);
    }

    function updateDesignImage() {
      ensureDesignImage();
      const img = platePreview.querySelector('.design-image');
      if (!img) return;
      const style = textStyleSelect ? textStyleSelect.value : null;
      const url = (style && styleImages[style]) ? styleImages[style] : '';
      if (url) { img.src = url; img.style.display = 'block'; }
      else { img.removeAttribute('src'); img.style.display = 'none'; }
    }
function updatePrice() {
      const count = container.querySelector('input[name="plate_count"]:checked')?.value;
      const style = textStyleSelect ? textStyleSelect.value : null;
      const sizeKey = plateSizeSelect ? plateSizeSelect.value : null;
      const surround = plateSurroundSelect ? plateSurroundSelect.value : 'none';

      // Style front/rear
      const fb = (style && stylePricesFB[style]) ? stylePricesFB[style] : {front:0, rear:0};
      let price = 0;
      if (count === 'both') price += (parseFloat(fb.front)||0) + (parseFloat(fb.rear)||0);
      else if (count === 'front') price += (parseFloat(fb.front)||0);
      else price += (parseFloat(fb.rear)||0);

      // Size add-ons
      const sz = (sizeKey && sizePricesFR[sizeKey]) ? sizePricesFR[sizeKey] : {front:0, rear:0};
      if (count === 'both') price += (parseFloat(sz.front)||0) + (parseFloat(sz.rear)||0);
      else if (count === 'front') price += (parseFloat(sz.front)||0);
      else price += (parseFloat(sz.rear)||0);

      // Surround
      if (surround && surround !== 'none') {
        const costs = surroundCosts[surround] || { single: 0, pair: 0 };
        price += (count === 'both') ? costs.pair : costs.single;
      }

      // Extras (sum once)
      let extrasTotal = 0;
      container.querySelectorAll('.extra-option:checked').forEach(chk => {
        const key = chk.value;
        if (extrasConfig[key]) extrasTotal += parseFloat(extrasConfig[key].price || 0);
      });
      price += extrasTotal;
      ...
      if (surround && surround !== 'none') {
        const costs = surroundCosts[surround] || { single: 0, pair: 0 };
        // removed stray platesNum usage; already handled above
      }
      priceEl.textContent = formatPrice(price);
    }
    ...
    if (plateSizeSelect && regInput) {
      ...
      plateSizeSelect.addEventListener('change', () => { applyMax(); renderAll(); applyMaxForSize();
        ensureExtrasUI();
      updatePrice();
      updateDesignImage(); updateFormFields(); });
      // Initialise on load
      applyMax();
    }
...
// Bind event listeners
    if (regInput) {
      regInput.addEventListener('input', () => {
        const mKey = plateSizeSelect ? plateSizeSelect.value : 'std';
        const m = getMaxForSize(mKey);
        const sanitised = regInput.value.toUpperCase().replace(/[^A-Z0-9 ]/g,'');
        const noSpaces = sanitised.replace(/\s+/g,'');
        regInput.value = noSpaces.slice(0, m);
