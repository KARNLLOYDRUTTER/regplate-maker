...
class WP_Plate_Builder_Widget extends Widget_Base {
    ...
    protected function register_controls() {
        ...
        // Price add-ons per plate size (per plate)
        $size_repeater->add_control(
            'price_front',
            [
                'label'   => __( 'Front price add-on', 'wp-plate-builder' ),
                'type'    => Controls_Manager::NUMBER,
                'default' => 0,
                'min'     => 0,
                'step'    => 0.01,
        ...
        $size_repeater->add_control(
            'price_rear',
            [
                'label'   => __( 'Rear price add-on', 'wp-plate-builder' ),
                'type'    => Controls_Manager::NUMBER,
                'default' => 0,
                'min'     => 0,
                'step'    => 0.01,
    ...
    $this->add_control(
        ...
        // Static preview image for this style (shows under plate preview)
        $style_repeater->add_control(
            'preview_image',
            [
    ...
    protected function render() {
        ...
        // Build maps for front/rear prices and images
        $style_prices_fb = [];
        $size_prices_fr  = [];
        $style_images    = [];
$style_prices = [];
        if ( ! empty( $settings['text_styles'] ) && is_array( $settings['text_styles'] ) ) {
            foreach ( $settings['text_styles'] as $style ) {
                ...
                $url = '';
                if ( isset($style['preview_image']) && is_array($style['preview_image']) && ! empty($style['preview_image']['url']) ) {
        ...
        $plate_sizes_map = [];
        $size_prices_fr = [];
        if ( ! empty( $settings['plate_sizes'] ) && is_array( $settings['plate_sizes'] ) ) {
